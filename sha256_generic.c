...
#define e0(x)       (ror32(x ^ ror32(x,11) ^ ror32(x,20), 2))
#define e1(x)       (ror32(x ^ ror32(x, 5) ^ ror32(x,19), 6))
  ...
  while (1) {
    ...
    *nonce = n;
    ...
    if (unlikely((n >= max_nonce) || thr->work_restart)) {
      ...
      applog(LOG_ERR, "Elapsed: %10llu cycles, Nonces: %7d, Per Nonce: %11.6f, Start nonce: %9d, Max nonce: %9d", elapsed, n - start_nonce, elapsed * 1.0 / (n - start_nonce), start_nonce, max_nonce);
